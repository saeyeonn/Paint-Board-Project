...
public class TextSize {
    ...
    public static void applySelectedAttributes() {
        ...
        if (selectedTextField != null) {
            TextState sizeState = statesMap.get(selectedTextField);

            if (sizeState != null) { // null 체크 추가
                Font font = selectedTextField.getFont();
                int size = 1;

                if ((int) TextSpinnerForm.getFontSizeSpinner().getValue() >= 0) {
                    size = sizeState.getFontSize(); // 선택한 크기 가져오기
                }

                System.out.println("사이즈"+ sizeState.getFontSize());

                Map<TextAttribute, Object> attributes = new HashMap<>();
                if (sizeState.isUnderline()) {
    ...
    public static void updateTextFieldSize(JTextField textField) {
        ...
        infoMap.get(textField).widthHeight(textWidth, textHeight);
    }

    // 현재 설정 저장
    private static void storeFontSize(JTextField textField) {
        if (textField != null && fontSizeSpinner != null) {
            TextState SizeStates = statesMap.get(textField);
            if (SizeStates == null) {
                SizeStates = new TextState();
    ...
    private static void loadFontSize(JTextField selectedTextField) {
        if (selectedTextField != null && fontSizeSpinner != null) {
            TextState SizeStates = statesMap.get(selectedTextField);
